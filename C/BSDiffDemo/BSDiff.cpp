...
Java_online_proyi_jbsdiff_utils_BsDiffUtil_fileDiff(JNIEnv *env, jobject thiz, jstring new_file_path, jstring old_file_path, jstring patch_file_path)
{
    ...
    char *argv[] = {"bs_diff", const_cast<char *>(oldFile), const_cast<char *>(newFile),
...
extern "C" JNIEXPORT jint JNICALL
Java_online_proyi_jbsdiff_utils_BsDiffUtil_filePatch(JNIEnv *env, jobject thiz, jstring old_file_path, jstring patch_file_path, jstring combine_file_path)
{
    const char *oldFile = env->GetStringUTFChars(old_file_path, nullptr);
    const char *patchFile = env->GetStringUTFChars(patch_file_path, nullptr);
    const char *combineFile = env->GetStringUTFChars(combine_file_path, nullptr);
    char *argv[] = {"bs_patch", const_cast<char *>(oldFile), const_cast<char *>(combineFile),
                    const_cast<char *>(patchFile)};
    /*调bspatch的main方法*/
